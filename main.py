...
cap = VideoStream(usePiCamera=True > 0).start()
...
#Opening (erode-> dilate) to remove noise.
    ...
    for cnt in contours0:
        ...
        if area > areaTH:
           ...
           #Falta add conditions for multipersons, outputs and screen entries.
...
#END while(cap.isOpened())
    ...
    data_to_up = {
        time.strftime("%Y-%m-%d %H:%M:%S"): up
    }
    data_to_down = {
        time.strftime("%Y-%m-%d %H:%M:%S"): down
    }

    result1 = FBConn.post('/Up/', data_to_up)
    result2 = FBConn.post('/Down/', data_to_down)
ser.close()
